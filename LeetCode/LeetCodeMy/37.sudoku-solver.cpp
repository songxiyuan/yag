...
class Solution {
public:
// check when every push down
    bool isValid(vector<vector<char> > &board, int x, int y)
	{
        //只需要检查当前数是否出现过就可以了，不需要建立哈希表
		int i, j;
		for (i = 0; i < 9; i++)
    ...
    bool dfs(vector<vector<char>>& board,int r,int l)
    {
        if(l==board[0].size())
        {
            if(r==board.size()-1)
            {
                return true;//用return ture的形式，可以让其在第一次找到正确答案的时候，就退出整个程序，不用遍历所有
            }
        ...
        if(board[r][l]=='.')
        {
            ...
            {
                board[r][l]=i+'0';
                if(isValid(board,r,l))
                {
                    if(dfs(board,r,l+1))return true;
                }
                board[r][l]='.';
            }
            return false;
