...
ListNode *deleteDuplication(ListNode *pHead) 
{
    ...
    {
        while (next != nullptr && pHead->m_nValue == next->m_nValue)
            next = next->m_pNext;
        return deleteDuplication(next);
    } 
    else 
    {
        pHead->m_pNext = deleteDuplication(pHead->m_pNext);
        return pHead;
    }
}
...
void Test2(char* testName, ListNode* pHead, int* expectedValues, int expectedLength)
{
    if(testName != nullptr)
        printf("%s begins: ", testName);
...
void Test0()
{
    ...
    Test2((char*)"Test0", pHead, expectedValues, sizeof(expectedValues) / sizeof(int));
